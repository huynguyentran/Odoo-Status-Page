...
def upgrade_module_list(check_version, result):
    ...
    module_list_upgrade_final = ','.join(
        list(filter(None, module_list_upgrade)))
    module_list_install_final = ','.join(
        list(filter(None, module_list_install)))
    ...
    if len(db_list_final) == 0:
        if len(module_list_upgrade_final) != 0 and len(module_list_install_final) != 0:
            ...
            file.write(odoo_upgrade + "/opt/odoo/odoo/odoo-bin --config /etc/odoo/odoo.conf --stop-after-init -i %s -u %s" %
                       (module_list_install_final, module_list_upgrade_final))
            file.close()
            ...
            print("Database: %s" % db_list_final)
            file = open("./odoo_upgrade.sh", "w")
            file.write(odoo_upgrade + "/opt/odoo/odoo/odoo-bin --config /etc/odoo/odoo.conf --stop-after-init -u %s" %
                       (module_list_upgrade_final))
            file.close()
            ...
            print("Database: %s" % db_list_final)
            file = open("./odoo_upgrade.sh", "w")
            file.write(odoo_upgrade + "/opt/odoo/odoo/odoo-bin --config /etc/odoo/odoo.conf --stop-after-init -i %s" %
                       (module_list_install_final))
            file.close()
        else:
...
if __name__ == "__main__":
    ...
    if version_list_deploy is None:
        ...
        upgrade_module_list(
            version_list_deploy['check_version'], version_list_deploy['result'])
